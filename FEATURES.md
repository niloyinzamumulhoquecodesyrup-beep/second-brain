...
**Reward panel** (`components/RewardPanel.js`): five dimensions, each a glass-tube "tank" gauge for today's count (Streak, Captures, Tasks done, Focus sessions, Focus time) plus a lifetime level number and a progress bar toward the next level. Levels only ever go up (pure function of lifetime totals from `GET /api/stats`, including `focusMinutesTotal`/`focusMinutesByDay` summed server-side from each logged session's real elapsed minutes); daily gauge targets adapt to the median of the last 7 days for that dimension rather than a fixed number, so the tank stays reachable. A streak counts consecutive active days ending today-or-yesterday (never reads as "broken" just because today hasn't happened yet). Eleven badges unlock at capture/task/focus/focus-time/streak milestones. Below the gauges, a "this week" / "this month" line sums the same day-by-day arrays over a wider trailing window. A rotating quote line (ADHD-aimed, about starting rather than effort) cycles daily. Completing a task or logging a focus session on this page bumps the gauges immediately client-side (no reload, minutes included for focus sessions) and has a small chance (or a guaranteed chance on a fresh level-up) of firing a bigger "surprise bonus" celebration with its own message. None of the reward state is persisted server-side beyond the underlying stats; levels/badges/streak are recomputed from real data every time.
...
For accuracy: a few features described in the repo's own planning documents are not fully live. The reminders/notifications system (`REMINDERS_PLAN.md`) is real as of Phase A: task due-dates auto-create reminders, a live bell badge + toast + Work-page nudge strip deliver them (respecting quiet hours and suppressing while a Pomodoro is running), and `routine_suggestion`-kind reminders can turn into real routines. What's still not live: Web Push (Phase B — works with the app closed, needs a service worker + VAPID keys + a cron dispatcher) and the two cycle-authored candidate kinds (`routine_suggestion`, `interest_event`) actually being populated day-to-day, which depends on a periodic Claude Code cycle run against real user data rather than application code. There is no browser extension or any device/browser activity tracking (`device_activity` stays empty by design). The multi-dimensional reward system described as a build spec (`REWARD_SYSTEM_PROMPT.md`) is implemented and live on the Work page. The full day/week planner UI (`ProductivityTab.js`) exists and is functional against real API routes but isn't linked from any page's navigation right now.
