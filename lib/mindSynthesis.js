...
// "How much" for an interest_cluster, alongside the existing "what" (the cluster
// itself) and "trend" (growing/fading direction). Mirror-not-oracle (§1): every
// factor here is a plain count/ratio already shown in the summary, not a hidden
// ML score — weight_factors is stored so the number stays traceable to its inputs
// instead of reading as an unexplained judgment.
// - recency: 1.0 if touched today, decays to 0 by twice the dormant threshold.
// - volume: log-scaled note count, saturating around 20 notes so one huge cluster
//   can't dominate every other signal.
// - momentum: share of the cluster's notes touched in the last RECENT_DAYS.
function interestWeight({ count, recentCount, lastActivityDays }) {
  const recency = Math.max(0, 1 - lastActivityDays / (DORMANT_DAYS * 2))
  const volume = Math.min(1, Math.log2(count + 1) / Math.log2(20))
  const momentum = count > 0 ? recentCount / count : 0
  const weight = Math.round((recency * 0.4 + volume * 0.35 + momentum * 0.25) * 100)
  const round2 = n => Math.round(n * 100) / 100
  return { weight, weight_factors: { recency: round2(recency), volume: round2(volume), momentum: round2(momentum) } }
}
...
async function tagBasedClusters(pool, userId) {
  ...
  for (const cluster of clusters) {
    ...
    const { weight, weight_factors } = interestWeight({ count: cluster.note_count, recentCount: cluster.recent_count, lastActivityDays })
    ...
    results.push({ summary, sourceRefs: notes.map(n => ({ type: 'note', id: n.id, title: n.title })), metadata: { method: 'tag', tag: cluster.tag, weight, weight_factors } })
  }
  return results
}
...
async function semanticClusters(pool, userId) {
  ...
  for (const ids of Object.values(groups)) {
    ...
    const { weight, weight_factors } = interestWeight({ count: notes.length, recentCount, lastActivityDays })
    ...
    results.push({ summary, sourceRefs: notes.map(n => ({ type: 'note', id: n.id, title: n.title })), metadata: { method: 'embedding', weight, weight_factors } })
  }
  return results
}
